...
2. **Resolve silently**: call `get_site_templates` and `get_environment_languages` behind the scenes. Present the results as friendly choices using `present_options` only if the user has not already specified the template or language.
